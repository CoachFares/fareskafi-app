...
export async function POST(req: NextRequest) {
  ...
  const prompt = buildStationOpenPrompt(stageId, summaries, lifeStage);

  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-api-key': process.env.ANTHROPIC_API_KEY, 'anthropic-version': '2023-06-01' },
      body: JSON.stringify({ model: 'claude-sonnet-5', max_tokens: 250, messages: [{ role: 'user', content: prompt }] }),
    });
    ...
    return NextResponse.json({ ok: true, question: parsed.question, source: 'ai' });
  } catch (err) {
