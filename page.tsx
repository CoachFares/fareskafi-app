...
export default function StartPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/start-journey', {
      ...
      .then(async (r) => {
        ...
        let data: any = {};
        try { data = JSON.parse(text); } catch { setDebugInfo(`status ${r.status}: ${text.slice(0, 300)}`); }
        if (!data.ok) {
          setDebugInfo(`status ${r.status}: ${text.slice(0, 300)}`);
          setPhase('invalid');
          return;
        }
  ...
  if (phase === 'invalid') {
    return <Center>هذا الرابط غير صالح أو منتهي. تأكد إنك تستخدم الرابط اللي وصلك بالبريد بالضبط.
      {debugInfo && <div style={{ marginTop: 16, fontSize: 12, color: '#b9552e', direction: 'ltr', textAlign: 'left' }}>{debugInfo}</div>}
    </Center>;
  }
  if (phase === 'building') {
